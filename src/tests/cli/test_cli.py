...
class TestCliExecutor:
    def test_cli_executor_file(self, mocker, generate_files):
        ...
        mock_ = mocker.patch.object(GeneratorsHandler, 'get_valid_specification')
        mock_.return_value = {}

        with runner.isolated_filesystem():
            with open('hello.json', 'w') as f:
                f.write("{{}}")
            result = runner.invoke(execute, ['generate', '--spec', 'hello.json'])
    ...
    def test_cli_executor_folder(self, mocker, generate_files):
        ...
        with runner.isolated_filesystem():
            os.mkdir('dir/')
            result = runner.invoke(execute, ['generate', '--spec', 'dir/'])
            assert result.exit_code != 0
