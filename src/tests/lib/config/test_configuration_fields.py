...
class TestConfigurationFormat(object):
    def test_fields_format(self):
        ...
        validator = ConfigurationFields(format_sample, 'pt_BR')
    ...
    def test_valid_boolean(self, valid_boolean):
        validator = ConfigurationFields(valid_boolean, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is True

    def test_invalid_boolean(self, invalid_boolean):
        validator = ConfigurationFields(invalid_boolean, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is False

    def test_valid_char(self, valid_char):
        validator = ConfigurationFields(valid_char, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is True

    def test_invalid_char(self, invalid_char):
        validator = ConfigurationFields(invalid_char, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is False

    def test_valid_float(self, valid_float):
        validator = ConfigurationFields(valid_float, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is True

    def test_invalid_float(self, invalid_float):
        validator = ConfigurationFields(invalid_float, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is False

    def test_valid_integer(self, valid_integer):
        validator = ConfigurationFields(valid_integer, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is True

    def test_invalid_integer(self, invalid_integer):
        validator = ConfigurationFields(invalid_integer, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is False

    def test_valid_timestamp_sequence(self, valid_timestamp_sequence):
        validator = ConfigurationFields(valid_timestamp_sequence, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is True

    def test_invalid_timestamp_sequence(self, invalid_timestamp_sequence):
        validator = ConfigurationFields(invalid_timestamp_sequence, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is False

    def test_valid_sequence(self, valid_sequence):
        validator = ConfigurationFields(valid_sequence, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is True

    def test_invalid_sequence(self, invalid_sequence):
        validator = ConfigurationFields(invalid_sequence, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is False

    def test_valid_string(self, valid_string):
        validator = ConfigurationFields(valid_string, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is True

    def test_invalid_string(self, invalid_string):
        validator = ConfigurationFields(invalid_string, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is False

    def test_valid_timestamp(self, valid_timestamp):
        validator = ConfigurationFields(valid_timestamp, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is True

    def test_invalid_timestamp(self, invalid_timestamp):
        validator = ConfigurationFields(invalid_timestamp, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is False

    def test_valid_name(self, valid_name):
        validator = ConfigurationFields(valid_name, 'pt_BR')
        is_valid = validator.is_valid()
        assert is_valid is True
